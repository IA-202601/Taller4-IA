...
def regress(goal_set: State, action: Action) -> State | None:
    ...
    delete_list = action.del_list
    preconditions = action.precond_pos

    if not (add_list & goal_set):
        return None

    if delete_list & goal_set:
        return None

    if action.precond_neg & goal_set:
        return None

    return frozenset((goal_set - add_list) | preconditions)
...
def backwardSearch(problem: Problem) -> list[Action]:
    ...
    def is_consistent_goal(goal):
        ...
        for fluent in goal:
            if fluent[0] == "Holding":
                ...
                if ("HandsFree", r) in goal:
                    return False

                if obj in at_by_entity:
                    return False
    ...
    visited = set()
    visited.add(initial_goal)

    domain = [action for action in problem.domain if action is not None]
    all_actions = get_all_groundings(domain, problem.objects)
    ...
    def valid_action(action):
        ...
        if name.startswith("Rescue"):
            return any(
                f[0] == "MedicalPost" and f in problem.initial_state
        ...
        if name.startswith("Move"):
            return any(
                f[0] == "Adjacent" and f in problem.initial_state
                for f in action.precond_pos
            )

        if name.startswith("PickUp"):
            return any(
                f[0] == "Pickable" and f in problem.initial_state
                for f in action.precond_pos
            )
        ...
        return False
    all_actions = [action for action in all_actions if valid_action(action)]
    ...
    while not frontier.isEmpty():
        ...
        for action in all_actions:
            ...
            if not is_consistent_goal(new_goal):
                continue

            impossible_static = False

            for fluent in new_goal:
                if fluent[0] in ["Adjacent", "MedicalPost", "Pickable"]:
                    if fluent not in problem.initial_state:
                        impossible_static = True
                        break
...
def aStarPlanner(
    ...
    h0 = heuristic(inicio, problem.goal, problem.domain, problem.objects)
    frontera.push((inicio, [], 0), h0)
    best_g = {inicio : 0}
    
    while not frontera.isEmpty():
        estado, plan, g = frontera.pop()
        ...
        for next_estado, accion, costo in problem.getSuccessors(estado):
            new_g = g + costo
            if new_g < best_g.get(next_estado, float("inf")):
                best_g[next_estado] = new_g
                h = heuristic(
                    next_estado, problem.goal, problem.domain, problem.objects
                )
                frontera.push((next_estado, plan + [accion], new_g), new_g + h)
